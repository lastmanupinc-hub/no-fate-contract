...
export interface RunOneResult {
  gridpoint_id: string;
  impl: string;
  per_action_outcome: string[];
  post_state_root: string;
  receipts_sha256: string[];
}

export async function runOne(adapter: IEngineAdapter, gp: GridPoint): Promise<RunOneResult> {
  ...
  if (gp.expected?.per_action_outcome) {
    assert(
      JSON.stringify(r.per_action_outcome) === JSON.stringify(gp.expected.per_action_outcome),
      `Outcome mismatch: expected=${JSON.stringify(gp.expected.per_action_outcome)} got=${JSON.stringify(r.per_action_outcome)}`
    );
  }
  ...
  if (gp.expected?.receipts_sha256) {
    assert(
      JSON.stringify(receipts_sha256) === JSON.stringify(gp.expected.receipts_sha256),
      `receipts_sha256 mismatch`
    );
  }
