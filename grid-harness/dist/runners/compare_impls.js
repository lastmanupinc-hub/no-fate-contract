...
export function compareImplementations(gridpointIds, implOutDirs) {
    for (const gpId of gridpointIds) {
        ...
        for (const impl of impls.slice(1)) {
            ...
            assert(JSON.stringify(cur.per_action_outcome) === JSON.stringify(first.per_action_outcome), `DIVERGENCE outcomes for ${gpId}: ${impl} vs ${impls[0]}`);
            assert(cur.post_state_root === first.post_state_root, `DIVERGENCE state_root for ${gpId}`);
            assert(JSON.stringify(cur.receipts_sha256) === JSON.stringify(first.receipts_sha256), `DIVERGENCE receipts_sha256 for ${gpId}`);
        }
    }
}
